var TableDatatablesAjax = function () {
    ...
    var handleExercicios = function (tipo) {
        ...
        grid.init({
            ...
            dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                ...
                "ajax": {
                    ...
                    "processing": false,
                    "url": "/usuario/api/list-usuario-ajax/",
                ...
                "columns": [
                    ...
                    { "data": "matricula", "defaultContent": "<i>-</i>" },
                    { "data": "first_name", "defaultContent": "<i>-</i>" },
                    { "data": "last_name", "defaultContent": "<i>-</i>" },
                    { "data": "email", "defaultContent": "<i>-</i>" },
                    { "data": "is_active", "defaultContent": "<i>-</i>" },
        ...
        $('table').on('click', '.PageLink', function(){
            ...
            // let colMatricula = $(this).parent().siblings()[1];
            let colNome = $(this).parent().siblings()[2];
            let colSobrenome = $(this).parent().siblings()[3];
            let colEmail = $(this).parent().siblings()[4];


            // $(colMatricula).html(`<input id="temp_mat" class="form-control" value="${rowData.matricula}" type="text" />`);
            $(colNome).html(`<input id="temp_first" class="form-control" value="${rowData.first_name}" type="text" />`);
            $(colSobrenome).html(`<input id="temp_last" class="form-control" value="${rowData.last_name}" type="text" />`);
            $(colEmail).html(`<input id="temp_email" class="form-control" value="${rowData.email}" type="text" />`);
            ...
            $('#cancelar').click(function () {
                cancelar(rowData, colNome, colSobrenome, colEmail, aux_this);
            });
        });
        ...
        function salvar(rowData) {
            $.ajax({
                method: 'GET',
                url: '/usuario/api/edit-usuario-ajax/',
                header: {'csrfmiddlewaretoken': csrf_token},
                data: {
                    'csrfmiddlewaretoken': csrf_token,
                    'id': rowData.id,
                    'first_name': $('#temp_first').val(),
                    'last_name': $('#temp_last').val(),
        ...
        function cancelar(rowData, colNome, colSobrenome, colEmail, aux_this) {
            $(colNome).html(rowData.first_name);
            $(colSobrenome).html(rowData.last_name);
            $(colEmail).html(rowData.email);
